...
<nav x-data="{ open: false, search:false }" class="bg-zinc-100 border-b border-gray-100 p-10 flex flex-col justify-between items-center space-y-10">
    <div class="container flex md:flex-col lg:flex-row justify-between items-center w-full mx-auto">
        ...
        @if(Auth::guard('acom')->user())
           <li>
              <a class="nav-link font-bold text-shokka-pink hover:text-shokka-blue-hover transition ease-in inline-flex py-2 px-3" href="{{ route('AcomLogout') }}" 
                onclick="event.preventDefault();
                document.getElementById('logout-form').submit();">
                <i class="nav-icon fas fa-fw fa-sign-out-alt"></i>
                {{ __('Sair') }}
              </a>
              <form id="logout-form" action="{{ route('AcomLogout') }}" method="POST" style="display: none;">
              @csrf </form>
           </li>
        @endif
            <li>
              <a href="{{ url('acompanhantes/login') }}" class="font-bold text-shokka-pink hover:text-shokka-blue-hover transition ease-in inline-flex py-2 px-3">
            ...
            <li>
              <a href="{{ url('publicar') }}" class="font-bold text-shokka-pink hover:text-shokka-blue-hover transition ease-in  inline-flex py-2 px-3">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 mr-1">
            ...
            <li>
              <a href="{{ url('publicar')}}" class="inline-flex py-2 px-5 text-white bg-shokka-blue transition ease-in hover:bg-shokka-blue-hover rounded"><span class="font-bold mr-1">+</span>PUBLICAR ANUNCIO</a>
            </li>
          </ul>
    </div>
    <ul class="flex flex-col gap-2 w-full" :class="open? 'transition ease-linear block' : 'hidden'">
      ...
      <li>
        <a href="{{ url('publicar')}}" class="block text-center py-2 px-5 text-white bg-shokka-blue transition ease-in hover:bg-shokka-blue-hover rounded"><span class="font-bold mr-1">+</span>PUBLICAR ANUNCIO</a>
      </li>
    </ul>
    <form action="{{ url('procurar-pesquisa') }}" method="post" class="hidden lg:block container w-full">
      @csrf
        ...
        <div class="flex items-center justify-center space-x-5 w-full">
            <select name="state" id="state" class="block border-zinc-200  font-medium text-gray-400 rounded w-1/2">
              @foreach ($states as $state)
              <option class="{{ $state->id }}" value="{{ $state->id }}">{{ $state->name }}</option>
            @endforeach
            </select>
            <select name="city" id="city" class="city block border-zinc-200  font-medium text-gray-400 rounded w-1/2">
              @foreach ($citys->where('state_id', 14) as $state)
                <option class="{{ $state->id }}" value="{{ $state->id }}">{{ $state->name }}</option>
              @endforeach
            </select>
