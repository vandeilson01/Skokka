...
<section class="text-center mb-5">
      <ul class="flex flex-wrap  mb-5 text-sm font-medium text-gray-500 sm:mb-0 dark:text-gray-400">
            ...
            <li>
               <a href="#" class="mr-4 hover:underline">
                  <button class="bg-transparent hover:bg-pink-500 text-pink-700 font-semibold hover:text-white py-2 px-4 border border-pink-500 hover:border-transparent rounded">
                     {{$states->where('id', $state)->first()->name}}
                  </button>
               </a>
            </li>
...
@php 
  $cidade = !empty($city) ? $city : 0;
  $estado = !empty($state) ? $state : 0;
  
  $get = App\Models\AnunciosModel::where('cidade', $cidade)->get() ?? App\Models\AnunciosModel::where('estado', $estado)->get();
@endphp

 @if($cidade != 0 || $estado != 0)

<section class="mt-5 mb-5">
   <div>{{ucfirst($all['categories'] ?? $categories)}} em {{ $citys->where('id', $cidade ?? $estado)->first()->name ?? '' }}</div>
   <div>{{date('d/m')}}</div>
</section>
